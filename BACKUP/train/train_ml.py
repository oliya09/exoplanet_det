...
# Disable all Torch compile features safely (works on all versions)
os.environ["TORCH_COMPILE_DISABLE"] = "1"
os.environ["TORCHDYNAMO_DISABLE"] = "1"

# Suppress compiler-related errors
...
def fetch_single_lightcurve(target, resolved_target):
    """Fetch a single lightcurve with caching."""
    ...
    for mission in ["TESS", "Kepler"]:
        try:
            ...
            # Pad or truncate to length 2000
            if len(flux) < 2000:
            ...
            logging.info(f"✅ Fetched {target} ({mission})")
            return flux

        except Exception as e:
            logging.warning(f"Failed {mission} for {search_target}: {e}")
            continue

    logging.error(f"❌ No LC for {target}")
    return None
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            for batch_x, batch_y in val_loader:
                ...
                val_preds.extend(probs.cpu().to(torch.float32).numpy().flatten())
