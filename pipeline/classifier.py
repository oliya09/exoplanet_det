...
class HybridClassifier:
    def __init__(self):
        """Загрузка CNN и RF моделей"""
        self.cnn_model = None
        self.input_len = 2000  # fallback длина

        # CNN
        if os.path.exists(CNN_MODEL_PATH):
            self.cnn_model = tf.keras.models.load_model(CNN_MODEL_PATH)
            try:
                # определить длину входа автоматически
                self.input_len = self.cnn_model.input_shape[1]
                print(f"[INFO] CNN модель загружена. Ожидаемая длина входа: {self.input_len}")
            except Exception:
                print("[WARN] Не удалось определить input_shape у CNN. Использую 2000.")
    ...
    def ml_score(self, flux):
        """Гибрид CNN + RF оценка вероятности планеты"""
        if hasattr(flux, 'filled'):
            flux = flux.filled(np.nan)
        flux = np.nan_to_num(np.array(flux, dtype=float), nan=1.0, posinf=1.0, neginf=1.0)

        # Подгоняем под длину CNN
        flux = flux[:self.input_len]
        if len(flux) < self.input_len:
            flux = np.pad(flux, (0, self.input_len - len(flux)), mode='constant')

        # Если нет CNN — используем только RF
        if self.cnn_model is None:
            features = flux.reshape(1, -1)
            return float(self.rf_model.predict_proba(features)[0, 1])

        # CNN
        flux_cnn = flux.reshape(1, self.input_len, 1)
        cnn_pred = float(self.cnn_model.predict(flux_cnn, verbose=0)[0][0])

        # RF
        rf_features = flux.reshape(1, -1)
        rf_pred = float(self.rf_model.predict_proba(rf_features)[0][1])
    ...
    def extract_features(self, lc, period, t0, duration, depth):
        ...
        centroid_var = np.var(flux) / np.mean(flux) if np.mean(flux) != 0 else 0
        features = [depth, period, duration, flux_rms, odd_even_diff, secondary_depth, centroid_var]
        return np.array(features), flux
    ...
    def classical_scores(self, lc, period, t0, duration, depth):
        ...
        hostname = get_hostname_from_tic("TIC_placeholder")
        ...
        return {
            ...
            'secondary': 'weak' if secondary_score else 'strong',
            'secondary_score': secondary_score,
            'centroid': centroid_flag,
            'centroid_score': centroid_score,
            'depth': 'reasonable' if depth_score else 'too_large',
            'depth_score': depth_score,
            'rv': 'ok' if rv_score else 'high',
            'rv_score': rv_score
        }
...
def classify_target_full(target_id, lc, period, t0, duration, depth, mission="TESS"):
    ...
    explain_parts = [
        f"ML: {ml_score:.2f}",
        f"Odd/even: {checks['odd_even']}",
        f"Secondary: {checks['secondary']}",
        f"Centroid: {checks['centroid']}",
        f"Depth: {checks['depth']}"
    ]
