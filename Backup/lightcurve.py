...
import time 
import json  # Для кэша star_params
import pickle  # Для кэша search results
warnings.filterwarnings("ignore", category=UserWarning)

# Константы для числового расчёта
...
def get_star_params(tic_id: str) -> Dict[str, float]:
    ...
    star_cache_file = f"cache/{tic_id.replace(' ', '_')}_star.json"  # Кэш для star_params
...
if __name__ == "__main__":
    # Работает с ЛЮБЫМ TIC ID
    tic_ids = ["TIC 150428135", "TIC 17361", "TIC 17362"]  # Пример с твоим ID
    for tid in tic_ids:
        # ✅ Ровно 7 значений — никаких ошибок unpack
